...
def get_latest_date(csv_path):
    df = pd.read_csv(csv_path, usecols= [2, 3, 4, 7])
    filt = (df['location'] == 'World')
    df = df[filt]
    lastrow = df.iloc[-1]
    return lastrow['date']
...
def merge_csv():
    ...
    target = "test_data/owid-covid-data.csv"
    ...
    filtered_df.to_csv(target, mode="a", index=False, header=False)

    sg.Popup('Dataset updated')
